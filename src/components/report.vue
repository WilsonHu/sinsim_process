          ...
          <div v-for="root in $router.options.routes" v-if="!root.hidden">
            <div v-for="sub in root.children" v-if="!sub.hidden">
              <el-menu-item
                ...
                :index="item.path"
                v-if="sub.path == '/home/report' && showSubMenu(item.path)"
                style="text-align: center; font-size: 14px; font-weight: bold"
              >{{item.meta}}</el-menu-item>
            </div>
          </div>
        </el-menu>
      </el-col>
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.code == 200) {
            ...
            if (_this.currentUserRoleScope.report.length > 0) {
              _this.$router.push(_this.currentUserRoleScope.report[0]);
            }
          } else {
