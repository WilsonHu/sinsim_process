<template xmlns:v-on="http://www.w3.org/1999/xhtml" xmlns:v-bind="http://www.w3.org/1999/xhtml">
    <el-container>
        <el-aside width="145px">
            <el-col>
                <el-menu style="background-color: transparent" :default-active="$route.path" theme="dark"
                         @select="handleSelect">
                    <div v-for="root in $router.options.routes" v-if="!root.hidden">
                        <div v-for="sub in root.children" v-if="!sub.hidden">
                            <el-menu-item v-for="item in sub.children" :index="item.path"
                                          v-if="sub.path == '/home/task' && showSubMenu(item.path)"
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.code == 200) {
                            ...
                            if(_this.currentUserRoleScope.task.length > 0) {
                                _this.$router.push(_this.currentUserRoleScope.task[0]);
                            }
                        } else {
