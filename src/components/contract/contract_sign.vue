<template>
  <div>
            ...
            >
              ...
              <el-table-column align="center" prop="status" label="审核状态">
                <template scope="scope">
                  ...
                  >{{filterContractStatus(scope.row.status)}}</div>
                </template>
              </el-table-column>
        ...
        <el-col :span="24">
          ...
          <el-card class="box-card" style="margin: 25px" v-if="isFinanceVisible()">
            ...
            <el-row style="margin-top: 10px" v-show="isFinanceVisible()">
              ...
              >优惠总价({{contractForm.currencyType}})：</el-col>
              <el-col
                :span="2"
                :offset="1"
                style="font-size: 24px;font-weight: bold; color: #409EFF"
              >{{calculateTotalDiscount()}}</el-col>
            </el-row>
            <el-row style="margin-top: 10px" v-show="isFinanceVisible()">
              <el-col
                :span="3"
                :offset="18"
                style="font-size: 16px;font-weight: bold;text-align: right;"
              >合同总价({{contractForm.currencyType}})：</el-col>
              <el-col
                :span="2"
                :offset="1"
                style="font-size: 28px;font-weight: bold; color: #F56C6C"
              >{{calculateTotalPrice()}}</el-col>
            </el-row>
            <br />
            <br />
                  ...
                  <div class="panel panel-primary">
                    ...
                    <div class="panel-body" v-if="isFinanceVisible()">
                      <table border="1" width="100%">
                        <tr>
                          <td colspan="2" style="font-weight: bold; font-size: 14px">机器价格（不含装置）</td>
                        ...
                        <tr>
                          <td colspan="2" style="font-weight: bold; font-size: 14px">订单总价（含装置）</td>
    ...
    <el-dialog title="删除" :visible.sync="deleteOrderConfirmVisible" width="30%" append-to-body>
      <span style="font-size: 22px">确认要删除该需求单吗？如果存在对应的机器则机器被设置为取消。</span>
      <span slot="footer" class="dialog-footer">
    ...
    showSplitOrChangeDeleteOrder(status) {
      if (
        ...
        (status == ORDER_CHECKING_FINISHED ||
          ...
          status == ORDER_SPLIT_FINISHED ||
          status == ORDER_CHANGE_FINISHED) && //允许二次多次改单拆单
...
//          && _this.contractForm.recordUser == _this.userInfo.account
    ...
    addTab(targetName) {
      ...
      let currentItem = copyObjectByJSON(
        _this.requisitionForms[_this.tabIndex - 2]
      );
      newItem.machineOrder = Object.assign({}, currentItem.machineOrder);
      newItem.orderDetail = Object.assign({}, currentItem.orderDetail);
      newItem.orderSign =
        copyObjectByJSON(currentItem.orderSign) != null
          ? copyObjectByJSON(currentItem.orderSign)
          : {};
...
//          machineNum:"",
    ...
    onAdd() {
      ...
      if (_this.isError) {
        ...
        for (let i = 0; i < obj.length; i++) {
          ...
          _this.requisitionForms[i].title =
            _this.requisitionForms[i].machineOrder.orderNum;
        }
        //设置合同属于哪一个销售组
    ...
    onSubmitContractSign(rowItem, signObj) {
      if (rowItem.comment == null || rowItem.comment == "") {
        ...
        $.ajax({
          ...
          dataType: "json",
          data: {
            contractSign: JSON.stringify(_this.editContractSign)
          },
          success: function(res) {
            if (res.code == 200) {
              showMessage(_this, "提交审核成功", 1);
    ...
    lxdSignDisable(row) {
      ...
      if (
        row.roleId == _this.userInfo.role.id &&
        _this.lxdForm.contactForm.status.indexOf("审核中") >= 0 &&
        row.shenHeEnabled
      ) {
        return false;
...
//              }
    ...
    getLxdChangeTypes() {
      if (
        _this.checkedChangeTypes == null ||
        _this.checkedChangeTypes.length == 0
      ) {
        return "";
      }
      //内容之间 用逗号隔开
    ...
    handleSubmitLxdSign(item) {
      if (item.comment == null || item.comment == "") {
        ...
        $.ajax({
          ...
          dataType: "json",
          data: {
            contactSign: JSON.stringify(_this.lxdForm.contactSign)
          },
          success: function(res) {
            if (res.code == 200) {
              showMessage(_this, "提交审核成功", 1);
